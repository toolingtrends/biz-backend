...
export async function listSpeakerFeedbackForAdmin(page = 1, limit = 50) {
  ...
  const eventIds = [...new Set(speakerSessions.map((s) => s.eventId).filter(Boolean))] as string[];
  ...
  const filtered = feedback.filter((f): f is NonNullable<typeof f> => f !== null);
...
export async function updateSpeakerFeedbackById(
  id: string,
  body: { isApproved?: boolean; isPublic?: boolean }
) {
