...
function validateYoutubeUrls(urls: unknown): urls is string[] {
...
export async function patchSessionHandler(req: Request, res: Response) {
  try {
    ...
    let youtubeUrls: string[] | undefined;
    if (body.youtube !== undefined) {
      ...
      youtubeUrls = Array.isArray(body.youtube)
        ? body.youtube.map((u: unknown) => String(u).trim())
        : undefined;
    }

    const updated = await prisma.speakerSession.update({
      where: { id },
      data: {
...
router.patch("/sessions/:id", requireUser, patchSessionHandler);
