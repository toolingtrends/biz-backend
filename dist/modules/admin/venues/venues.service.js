...
function mapVenueEventsForAdmin(rows, fallbackVenueId) {
    return rows.map((e) => ({
        id: e.id,
        title: e.title,
...
function spaceCapacityFromRow(row) {
    const r = row;
    for (const key of ["capacity", "maxCapacity", "seatingCapacity", "maxAttendees"]) {
        if (!(key in r))
            continue;
...
function inferFromMeetingSpaces(meetingSpaces) {
    ...
    for (const row of meetingSpaces) {
        if (row && typeof row === "object")
            capacity += spaceCapacityFromRow(row);
    }
...
function resolveCapacityAndHalls(maxCapacity, totalHalls, meetingSpaces) {
    const inferred = inferFromMeetingSpaces(meetingSpaces);
    let cap = maxCapacity ?? 0;
    let halls = totalHalls ?? 0;
    if (!cap && inferred.capacity > 0)
        cap = inferred.capacity;
    if (!halls && inferred.halls > 0)
        halls = inferred.halls;
...
async function syncLocationMasterFromVenue(input) {
    ...
    if (!cityName || !stateName)
        return;
    const city = await prisma_1.default.city.findFirst({
        where: {
            countryId: country.id,
...
async function listVenues(query) {
    ...
    const data = items.map((u) => {
        const { maxCapacity, totalHalls } = resolveCapacityAndHalls(u.maxCapacity, u.totalHalls, u.meetingSpaces);
        return {
            id: u.id,
            ...
            totalEvents: (u._count?.venueEvents ?? 0) || u.totalEvents || 0,
            isActive: u.isActive,
            createdAt: u.createdAt.toISOString(),
            updatedAt: u.updatedAt.toISOString(),
            events: mapVenueEventsForAdmin(u.venueEvents ?? [], u.id),
        };
    });
...
async function getVenueById(id) {
    ...
    if (!user)
        return null;
    const { maxCapacity, totalHalls } = resolveCapacityAndHalls(user.maxCapacity, user.totalHalls, user.meetingSpaces);
    const venueEventsList = user.venueEvents ?? [];
    return {
        id: user.id,
        ...
        totalEvents: (user._count?.venueEvents ?? 0) || user.totalEvents || 0,
